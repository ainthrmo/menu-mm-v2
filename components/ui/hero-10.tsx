...
export function Hero10({
  ...
  const titleElement = title && (
    <h1
      className={cn(
        'font-fraunces font-medium tracking-tight text-balance text-inherit',
        vs.title,
      )}
    >
      ...
      {(titleLine2Prefix || titleHighlight) && (
        <>
          ...
          <Balancer>
            ...
            {titleHighlight && (
              <span className="text-[#c8f04a] font-semibold">{titleHighlight}</span>
            )}
          </Balancer>
        </>
      )}
  ...
  const descriptionElement = description && (
    <p className={cn('text-[#c2c8b8] leading-relaxed', vs.description)}>
  ...
  const socialProofElement = socialProof && (
    <p className="text-[#a4ad9a] text-xs sm:text-sm font-medium">{socialProof}</p>
  )
